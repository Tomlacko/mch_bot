...
async def getIssueData(issueID: str) -> dict:
    ...
    data["project_name"] = tryOrDefault(
        lambda: x.project.name, "(Unknown project)"
    ...
    data["title"] = tryOrDefault(
        lambda: x.summary, "(Unknown issue title)"
    )  # title of the issue
    data["description"] = tryOrDefault(
        lambda: x.description, ""
    )  # the whole description with html-like formatting
    data["type"] = tryOrDefault(lambda: x.issuetype, "(Unknown)")  # always Bug
    data["status"] = tryOrDefault(
        lambda: x.status.name, "(Unknown)"
    )  # status: Resolved
    ...
    data["reporter"] = tryOrDefault(
        lambda: x.reporter.displayName, "(Unknown)"
    )  # [role] username
    data["reporter_icon"] = tryOrDefault(
        lambda: getattr(x.reporter.avatarUrls, "48x48"),
        "https://bugs.mojang.com/secure/useravatar?size=small&avatarId=10123",
    )  # user avatar icon
    data["created_time"] = tryOrDefault(
        lambda: round(dateparser.parse(x.created).timestamp()), None
    )  # unix timestamp
    data["assignee"] = tryOrDefault(
        lambda: x.assignee.displayName, "(Unassigned)"
    )  # [Mojang] username
    data["assignee_icon"] = tryOrDefault(
        lambda: getattr(x.assignee.avatarUrls, "48x48"),
        "https://bugs.mojang.com/secure/useravatar?size=small&avatarId=10123",
    )  # user avatar icon
    data["resolution"] = tryOrDefault(lambda: x.resolution.name, "Unresolved")  #'Fixed'
    data["resolution_time"] = tryOrDefault(
        lambda: round(dateparser.parse(x.resolutiondate).timestamp()), None
    )  # unix timestamp
    data["last_updated"] = tryOrDefault(
        lambda: round(dateparser.parse(x.updated).timestamp()), None
    )  # unix timestamp
    data["category"] = tryOrDefault(
        lambda: ", ".join([c.value for c in x.customfield_11901]), "(None)"
    )  # Category: 0: Entities, 1:...
    data["labels"] = tryOrDefault(
        lambda: (", ".join(x.labels)) if x.lables else "(None)", "(None)"
    )  # labels

    data["linked_issues"] = tryOrDefault(
        lambda: len(x.issuelinks), 0
    )  # total linked issues
...
async def getIssueDataMC(issueID: str) -> dict:
    ...
    data["confirmation"] = tryOrDefault(
        lambda: x.customfield_10500.value, "(Unknown)"
    )  # confirmation status: Confirmed
    data["priority"] = tryOrDefault(
        lambda: x.customfield_12200.value, "(Unspecified)"
    )  # mojang priority: Important

    data["fix_version"] = tryOrDefault(
        lambda: x.fixVersions[-1].name, "(None)"
    )  # last version to which a fix applies
    data["first_affected_version"] = tryOrDefault(
        lambda: x.versions[0].name, "(Unknown)"
    )  # first version that was affected
    data["last_affected_version"] = tryOrDefault(
        lambda: x.versions[-1].name, "(Unknown)"
    )  # last version that was affected
...
class MojiraEmbeds(commands.Cog):
    ...
    async def mojiraCommandError(
        ...
        if (
            ...
            await ctx.reply(
                f"Command failed - bad formatting.\nUse `{self.bot.config.bot_prefix}mojira <bug-id>` (Example of bug-id: `MC-1337`).",
                mention_author=False,
            )
        else:
            await ctx.reply("Command failed.", mention_author=False)

    async def showMojiraEmbed(
        self, ctx: commands.Context, issueID: str, hideErrors: bool = True
    ):
        """Posts an embed with details about the issue."""
        ...
        if not projectID:
            if not hideErrors:
                await ctx.reply(
                    "Command failed - invalid issue ID!", mention_author=False
                )
            return
        ...
        if projectID == "MC":
            ...
            if not data:
                if not hideErrors:
                    await ctx.reply(
                        f"Command failed:\nFailed to obtain issue data from <https://bugs.mojang.com/browse/{issueID}>",
                        mention_author=False,
                    )
                return
...
**Resolution date:**
{(f"<t:{data['resolution_time']}:d> <t:{data['resolution_time']}:t>{NEW_LINE}(<t:{data['resolution_time']}:R>)") if data["resolution_time"] else "(None)"}""",
                inline=True,
            )
...
(<t:{data["created_time"]}:R>)""",
            ...
            embed.add_field(
                ...
                value=f"""{data["first_affected_version"]}
...
{data["comments"]} comment{pluralSuffix(data['comments'])} • {data["watchers"]} watcher{pluralSuffix(data["watchers"])}"""
            ...
            if not hideErrors:
                await ctx.reply(
                    "Command failed - Issues from the specified project aren't supported yet, sorry!",
                    mention_author=False,
                )
        else:
            if not hideErrors:
                await ctx.reply(
                    "Command failed - Invalid project ID!", mention_author=False
                )
