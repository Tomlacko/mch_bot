...
class FunCommands(commands.Cog):
    ...
    async def spamping(
        self, ctx: commands.Context, user: discord.User, amount: int = 3, delay: int = 2
    ):
        ...
        if await self.bot.is_owner(ctx.author):
            ...
            if self.spampinging:
                await ctx.reply(
                    "Wait until the previous spampinging finishes...",
            ...
            for i in range(amount):
                await ctx.send(
                    f"{ctx.author.mention} {'🏓' if ctx.author==user else 'no u'}"
                )
                await asyncio.sleep(delay)
            self.spampinging = False
    ...
    async def spampingError(self, ctx: commands.Context, error: commands.CommandError):
        await ctx.reply(
            f"Command failed.\nUse `{self.bot.config.bot_prefix}spamping <user> [amount (default=3, min=1, max=5)] [delay (default=2, min=1, max=5)]`.",
            mention_author=False,
        )
