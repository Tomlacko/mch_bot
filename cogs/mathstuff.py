...
class MathStuff(commands.Cog):
    ...
    async def mathCommand(self, ctx: commands.Context, *, expression: str = ""):
        ...
        await ctx.reply(
            "Not implemented yet, coming soon! (hopefully).", mention_author=False
        )

    @mathCommand.error
    async def mathCommandError(
    ...
    async def randomChoice(self, ctx: commands.Context, *choices: str):
        ...
        if len(choices) == 1:
            await ctx.reply(
                f"After some extremely randomized choosing from the one singular option that was given to choose from, the surprising result is:\n{choices[0]}",
                mention_author=False,
            )
        else:
            await ctx.reply(
                f"Randomly chosen result:\n{random.choice(choices)}",
                mention_author=False,
            )

    @commands.command(name="randomint", aliases=["randomnum"])
    async def randomInt(
        self, ctx: commands.Context, start: int = None, stop: int = None
    ):
    ...
    async def randomFloat(
        self, ctx: commands.Context, start: float = None, stop: float = None
    ):
